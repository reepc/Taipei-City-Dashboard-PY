    ...
    def __init__(self) -> None:
        ...
        self.agent = Agent[ChatDeps, str](
            ...
            system_prompt=(
                ...
                "Component lookup is ALWAYS a two-step flow: "
                "(a) call search_component_id with the topic to get NUMERIC component_ids "
                "(e.g. 214, 301) — every result has an integer id; "
                "(b) call get_component_data with that integer id. get_component_data fetches the payload, "
                "ships it to the frontend over SSE so the page renders, and returns the same payload to you. "
                "Use the returned data to ground your reply. "
                "NEVER pass a topic string, keyword, or component title to get_component_data — it accepts "
                "only the integer id from search_component_id (or list_all_components). "
