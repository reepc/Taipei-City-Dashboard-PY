    ...
    def __init__(self) -> None:
        ...
        self.agent = Agent[ChatDeps, str](
            ...
            system_prompt=(
                ...
                "\n"
                "Mode layer (always on for trip queries):\n"
                "- Every trip query turns on a mode-specific map layer so the relevant "
                "infrastructure is visible alongside the route. Pick by transport mode:\n"
                "    • driving / 開車 → search_component_id(\"公共停車場\") then "
                "add_map_component on a returned id (parking-lot layer);\n"
                "    • YouBike / Ubike / 騎 YouBike → search_component_id(\"YouBike\") "
                "then add_map_component on a returned id (station layer);\n"
                "    • walking / 走路 / 步行 → search_component_id(\"人行道\") then "
                "add_map_component on a returned id (pavement layer).\n"
                "- Always add_map_component only — never add_card_in_chat for trip-mode "
                "layers (the user is route-planning, not browsing the dataset).\n"
                "- Place this between goto_coordinate and the mode's availability / navigate "
                "call. It runs alongside get_parking_availability / get_youbike_availability / "
                "navigate, not instead of them.\n"
                "\n"
                "Grounding:\n"
                ...
                "  search_component_id(query=\"YouBike\") →\n"
                "    [{component_id: <youbike_id>, name: \"YouBike使用情況\", ...}, ...]\n"
                "  add_map_component(component_id=<youbike_id>)            # YouBike station layer\n"
                ...
                "  search_component_id(query=\"公共停車場\") →\n"
                "    [{component_id: <parking_id>, name: \"公共停車場\", ...}, ...]\n"
                "  add_map_component(component_id=<parking_id>)            # parking-lot layer\n"
                ...
                "  search_component_id(query=\"人行道\") →\n"
                "    [{component_id: <pavement_id>, name: \"人行道分布\", ...}, ...]\n"
                "  add_map_component(component_id=<pavement_id>)            # pavement layer\n"
                ...
                "  search_component_id(query=\"停車場\") →\n"
                "    [{component_id: <id>, name: \"公共停車場\", ...}, ...]\n"
                "  add_map_component(component_id=<id>)            # turn on the parking-lot layer\n"
                "  add_card_in_chat(component_id=<id>)             # same dataset as a side panel\n"
                "Reply: 已將地圖移到西門並開啟公共停車場圖層，旁邊面板也列出了停車場資訊。\n"
                "Note: do NOT call get_parking_availability here. Once the camera is focused on "
                "the place and the map layer is on, the frontend renders the parking lots in that "
                "area on its own — the agent does not fetch per-place parking data. "
                "get_parking_availability is reserved for \"X 好停車嗎?\" availability-pressure "
                "questions (Example 2), where the model needs aggregate numbers to answer in text.\n"
                "\n"
